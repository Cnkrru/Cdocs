#!/usr/bin/env python3
# vercel 部署插件：生成 vercel.json + .github/workflows/build-linux-binary.yml
#
# 背景：Vercel 云端构建环境没有 C++ 编译器（现场编译失败过），
# 方案 = GitHub Actions 预编译 Linux 版 Cdocs-linux 二进制提交回仓库，
  ...
  build:
    ...
    steps:
      ...
      - name: 编译 Linux 版 Cdocs（全静态链接，跨发行版可运行）
        run: bash .Cdocs/tools/build.sh

      - name: 提交 Cdocs-linux 到仓库
        run: |
          cp Cdocs Cdocs-linux
          chmod +x Cdocs-linux
...
def main():
    ...
    # 检查 Cdocs-linux 二进制是否已存在（首次 setup 时提示）
    binp = os.path.join(source, "Cdocs-linux")
    hint = ""
    if not os.path.exists(binp):
        hint = "（注意：Cdocs-linux 不存在——push 后 Actions 会编译生成并提交，首次 Vercel 部署会失败一次，随后自动成功）"
