...
Cdocs 支持**多版本文档站**：把当前 `md/` 复制一份快照为 `md-v1/`、`md-v2/`，构建时自动识别为历史版本，生成 `current`（最新）与各历史版本，页眉出现**版本下拉**一键切换。该功能**默认启用、约定优于配置**——不写任何配置就能用。

## 约定：目录即版本

| 目录 | 版本名 | 说明 |
|------|--------|------|
| `md/` | `current` | 最新版本，页眉标「最新」 |
| `md-v1/` | `v1` | 历史版本（`docs-<名字>` 都会被识别） |
| `md-v2/` | `v2` | 依此类推 |
...
- `name`：版本目录名（`md-<name>` 或 `current` 指当前 `md/`）
...
> 注意：`md-v1` 是内容快照，纳入版本管理或随站点发布均可；删除 `md-v1` 后重新构建即回到单版本，**零残留**。
